...
- Unattended mission runner: `MISSION-RUNNER.bat` (wraps `mission-runner.ps1`, polls `/api/state` every 60s, dispatches headless `Codex -p "/mission ..."` — human approval gate via dashboard before it executes).
...
- `runtime.json` `executor` field can be set to `"failover"` to route a mission through the Codex→Codex→Gemma chain instead of plain headless Codex.
